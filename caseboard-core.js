(function(root){
 ...
 function validate(s){
  ...
  const nodes=s.nodes.map(n=>{if(!n||!text(n.id,100,false)||ids.has(n.id)||!text(n.name,100,false)||!['person','group','place'].includes(n.kind)||!text(n.race,100)||!text(n.role,100)||!text(n.status,100)||!text(n.notes,4000)||!text(n.appearance,2000)||!text(n.portrait,500)||!Number.isFinite(n.x)||!Number.isFinite(n.y)||n.x<100||n.x>width-100||n.y<110||n.y>height-110||n.portrait&&!/^assets\/images\/[a-zA-Z0-9_./-]+$/.test(n.portrait)||n.portrait.includes('..'))throw Error('人物资料或位置无效');ids.add(n.id);return {id:n.id,kind:n.kind,name:n.name,race:n.race,role:n.role,status:n.status,notes:n.notes,appearance:n.appearance,portrait:n.portrait,x:n.x,y:n.y,...(n.style===undefined?{}:{style:style(n.style,'node')})}});
  ...
  const zoneIds=new Set(),zones=(s.zones||[]).map(z=>{if(!z||!text(z.id,100,false)||zoneIds.has(z.id)||!text(z.name,100,false)||!['sage','amber','lilac'].includes(z.tone)||!['x','y','width','height'].every(k=>Number.isFinite(z[k]))||z.x<0||z.y<0||z.width<240||z.height<200||z.x+z.width>width||z.y+z.height>height)throw Error('结界范围无效，请保持在案件板内（最小 240 × 200）');zoneIds.add(z.id);return {id:z.id,name:z.name,tone:z.tone,x:z.x,y:z.y,width:z.width,height:z.height}});
  return {version:1,boardId:'black-tower',nodes,edges,zones};
 }
 const api={width,height,validate,style,defaults:copy(defaults),initial:()=>validate(root.CASEBOARD_SEED),copy};if(typeof module==='object'&&module.exports)module.exports=api;else root.CASEBOARD_CORE=api;
