(function(root){
 ...
 function personPreview(n){
  const match=n.role.match(/\s*[·・]\s*Lv\.(\d+)\s*$/i),legacy=match?Number(match[1]):null;
  const recognized=legacy!==null&&legacy>=1&&legacy<=20;
  return {role:recognized?n.role.slice(0,match.index).trim():n.role,level:n.level===undefined?(recognized?legacy:null):n.level,status:['死亡','已故'].includes(n.status.trim())?'已故':n.status.trim()};
 }
 ...
 function validate(s){
  ...
  const nodes=s.nodes.map(n=>{if(!n||!text(n.id,100,false)||ids.has(n.id)||!text(n.name,100,false)||!['person','group','place'].includes(n.kind)||!text(n.race,100)||!text(n.role,100)||!text(n.status,100)||!text(n.notes,4000)||!text(n.appearance,2000)||!text(n.portrait,500)||!Number.isFinite(n.x)||!Number.isFinite(n.y)||n.portrait&&!/^assets\/images\/[a-zA-Z0-9_./-]+$/.test(n.portrait)||n.portrait.includes('..'))throw Error('人物资料或位置无效');if(n.level!==undefined&&n.level!==null&&(!Number.isInteger(n.level)||n.level<1||n.level>20))throw Error('等级必须是 1–20 或未知');if(n.mapPieceId!==undefined&&n.mapPieceId!==null&&!text(n.mapPieceId,100,false))throw Error('地图地点关联无效');ids.add(n.id);return {id:n.id,kind:n.kind,name:n.name,race:n.race,role:n.role,status:n.status,notes:n.notes,appearance:n.appearance,portrait:n.portrait,x:n.x,y:n.y,...(n.mapPieceId===undefined?{}:{mapPieceId:n.mapPieceId}),...(n.level===undefined?{}:{level:n.level}),...(n.style===undefined?{}:{style:style(n.style,'node')})}});
 ...
 const api={validate,style,personPreview,defaults:copy(defaults),initial:()=>validate(root.CASEBOARD_SEED),copy};if(typeof module==='object'&&module.exports)module.exports=api;else root.CASEBOARD_CORE=api;
