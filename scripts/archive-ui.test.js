...
test('map creates or links a person at its selected piece; board reads location and can unlink without map writes',async()=>{
 const server=copy(seed),before=copy(server.parts),piece=server.parts.map.snapshot.pieces.find(p=>p.kind==='village'),a=page('map.html',server,{storedKey:'a'.repeat(64),fragment:'?piece='+piece.id});
 try{await a.ready();await wait(()=>a.q('#pointTitle').textContent===piece.name);const clickText=text=>[...a.q('#piecePeople').querySelectorAll('button')].find(b=>b.textContent===text).click();
  clickText('在此新建人物');assert(a.q('#mapPersonLocation').textContent.includes(piece.name));a.q('#mapPersonCancel').click();assert.equal(a.calls.length,0);
  clickText('在此新建人物');a.q('#mapPersonName').value='地点证人';a.q('#mapPersonRole').value='游侠';a.q('#mapPersonLevel').value='5';a.q('#mapPersonNotes').value='<b>纯文本</b>';a.q('#mapPersonForm').dispatchEvent(new a.w.Event('submit',{cancelable:true}));await a.w.ADVENTURE_ARCHIVE.stores.get('board').sync();
  const created=server.parts.board.snapshot.nodes.find(n=>n.name==='地点证人');assert.equal(created.mapPieceId,piece.id);assert.equal(created.level,5);assert(a.q('#piecePeople').textContent.includes('游侠 · 等级 5'));assert.equal(a.q('#piecePeople b'),null);assert.equal(a.q('#piecePeople .piece-person').getAttribute('href'),'./caseboard.html?person='+created.id);
  clickText('在此新建人物');a.q('#mapPersonName').value='地点证人';a.q('#mapPersonForm').dispatchEvent(new a.w.Event('submit',{cancelable:true}));assert(a.q('#mapPersonError').textContent.includes('已有同名'));assert.equal(a.w.CaseboardAPI.read().nodes.filter(n=>n.name==='地点证人').length,1);a.q('#mapPersonCancel').click();
  clickText('关联已有人物');a.q('#mapPersonExisting').value='gandalf';a.q('#mapPersonForm').dispatchEvent(new a.w.Event('submit',{cancelable:true}));await a.w.ADVENTURE_ARCHIVE.stores.get('board').sync();assert.equal(server.parts.board.snapshot.nodes.find(n=>n.id==='gandalf').mapPieceId,piece.id);
  const b=page('caseboard.html',server,{storedKey:'a'.repeat(64),fragment:'?person='+created.id});try{await b.ready();assert.equal(b.q('#boardDetailTitle').textContent,'地点证人');assert.equal(b.q('.person-map-location a').textContent,piece.name);assert(b.q('.person-map-location a').href.endsWith('piece='+piece.id));b.q('#boardEdit').click();b.q('#boardModify').click();assert.equal(b.q('#boardMapPiece').value,piece.id);b.q('#boardMapPiece').value='';b.q('#boardPersonForm').dispatchEvent(new b.w.Event('submit',{cancelable:true}));await b.w.ADVENTURE_ARCHIVE.stores.get('board').sync();assert.equal(server.parts.board.snapshot.nodes.find(n=>n.id===created.id).mapPieceId,null)}finally{b.w.close()}
  for(const part of ['character','map','journal'])assert.deepEqual(server.parts[part],before[part]);
 }finally{a.w.close()}
});

test('linked location follows map rename and deletion without losing the person or copying coordinates',async()=>{
 const server=copy(seed),piece=server.parts.map.snapshot.pieces[1];server.parts.board.snapshot.nodes.find(n=>n.id==='gandalf').mapPieceId=piece.id;const a=page('caseboard.html',server,{fragment:'?person=gandalf'});
 try{await a.ready();const original=copy(a.w.CaseboardAPI.read());piece.name='改名后的地点';piece.x+=20;server.parts.map.revision++;server.revision++;await a.w.ADVENTURE_ARCHIVE.getLatest();assert.equal(a.q('.person-map-location a').textContent,piece.name);server.parts.map.snapshot.pieces=server.parts.map.snapshot.pieces.filter(p=>p.id!==piece.id);server.parts.map.revision++;server.revision++;await a.w.ADVENTURE_ARCHIVE.getLatest();assert(a.q('.person-map-location').textContent.includes('已移除'));assert.deepEqual(copy(a.w.CaseboardAPI.read()),original);assert.equal(a.calls.length,0)}finally{a.w.close()}
});
