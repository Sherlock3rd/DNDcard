...
test('wards validate bounds, preserve empty groups and upgrade legacy snapshots without touching cards',()=>{
 ...
 for(const mutate of [s=>s.zones=null,s=>s.zones.push({...s.zones[0]}),s=>s.zones[0].width=NaN,s=>s.zones[0].x=-1,s=>s.zones[0].height=199,s=>s.zones[0].width=1800,s=>s.zones[0].tone='red',s=>s.zones[0].name='']){const s=seed();mutate(s);assert.throws(()=>core.validate(s))}
});
