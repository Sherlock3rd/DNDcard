...
test('Raven Town adds screenshot facts without changing existing records or making uncertain allies certain',()=>{
 ...
 const bad=JSON.parse(JSON.stringify(next));bad.nodes.at(-1).x=core.width;assert.throws(()=>core.validate(bad));
});
...
test('wards validate bounds, preserve empty groups and upgrade legacy snapshots without touching cards',()=>{
 ...
 for(const mutate of [s=>s.zones=null,s=>s.zones.push({...s.zones[0]}),s=>s.zones[0].width=NaN,s=>s.zones[0].x=-1,s=>s.zones[0].height=199,s=>s.zones[0].width=core.width,s=>s.zones[0].tone='red',s=>s.zones[0].name='']){const s=seed();mutate(s);assert.throws(()=>core.validate(s))}
});
