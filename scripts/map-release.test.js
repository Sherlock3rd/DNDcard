...
test('release picker offers 3 per category, one traveler, placement and details work',()=>{
 ...
 }assert.equal(placements,12);assert.equal(w.document.querySelectorAll('.piece').length,13);assert(q('.map-return').getAttribute('href').includes('index.html'));assert(q('#mapSaveStatus'));assert(!q('footer').textContent.includes('刷新后重置'));
 }finally{w.close()}
});
