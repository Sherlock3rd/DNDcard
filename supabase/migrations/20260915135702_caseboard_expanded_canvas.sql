-- Expand the drawable area without moving existing records or changing their fields.
...
begin
 ...
 if s ? 'zones' then
  ...
  for z in select value from jsonb_array_elements(s->'zones') loop
   if not ((jsonb_typeof(z)='object' and jsonb_typeof(z->'id')='string' and length(trim(z->>'id')) between 1 and 100 and jsonb_typeof(z->'name')='string' and length(trim(z->>'name')) between 1 and 100 and z->>'tone' in ('sage','amber','lilac') and jsonb_typeof(z->'x')='number' and jsonb_typeof(z->'y')='number' and jsonb_typeof(z->'width')='number' and jsonb_typeof(z->'height')='number' and (z->>'x')::numeric>=0 and (z->>'y')::numeric>=0 and (z->>'width')::numeric>=240 and (z->>'height')::numeric>=200 and (z->>'x')::numeric+(z->>'width')::numeric<=3600 and (z->>'y')::numeric+(z->>'height')::numeric<=1600) is true) then return false;end if;
