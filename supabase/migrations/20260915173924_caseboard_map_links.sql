...
begin
 ...
 for n in select value from jsonb_array_elements(s->'nodes') loop
  ...
  if n ? 'level' and not ((n->'level'='null'::jsonb or (jsonb_typeof(n->'level')='number' and (n->>'level')::numeric between 1 and 20 and (n->>'level')::numeric=trunc((n->>'level')::numeric))) is true) then return false;end if;
  if n ? 'mapPieceId' and not ((n->'mapPieceId'='null'::jsonb or (jsonb_typeof(n->'mapPieceId')='string' and length(trim(n->>'mapPieceId')) between 1 and 100)) is true) then return false;end if;
...
-- Older clients preserve links; explicit null unlinks. No coordinate duplication.
create function adventure_private.keep_board_map_links() returns trigger language plpgsql set search_path='' as $$
declare rebuilt jsonb;
begin
 if jsonb_typeof(new.snapshot->'nodes')='array' then
  select coalesce(jsonb_agg(case when not (incoming.value ? 'mapPieceId') and previous.value ? 'mapPieceId' then jsonb_set(incoming.value,'{mapPieceId}',previous.value->'mapPieceId') else incoming.value end order by incoming.ordinality),'[]'::jsonb)
  into rebuilt from jsonb_array_elements(new.snapshot->'nodes') with ordinality incoming(value,ordinality)
  left join lateral (select value from jsonb_array_elements(old.snapshot->'nodes') where value->>'id'=incoming.value->>'id' limit 1) previous on true;
  new.snapshot:=jsonb_set(new.snapshot,'{nodes}',rebuilt);
 end if;
 return new;
end;$$;
revoke all on function adventure_private.keep_board_map_links() from public,anon,authenticated;
create trigger board_keep_map_links before update on public.board_saves for each row execute function adventure_private.keep_board_map_links();
