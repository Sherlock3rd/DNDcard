...
-- Missing means an older client; explicit JSON null means the owner selected ?.
create function adventure_private.keep_board_levels() returns trigger language plpgsql set search_path='' as $$
declare rebuilt jsonb;
begin
 if jsonb_typeof(new.snapshot->'nodes')='array' then
  select coalesce(jsonb_agg(case when not (incoming.value ? 'level') and previous.value ? 'level' then jsonb_set(incoming.value,'{level}',previous.value->'level') else incoming.value end order by incoming.ordinality),'[]'::jsonb)
...
revoke all on function adventure_private.keep_board_levels() from public,anon,authenticated;
create trigger board_keep_levels before update on public.board_saves for each row execute function adventure_private.keep_board_levels();
