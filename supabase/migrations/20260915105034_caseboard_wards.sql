...
-- Old cached clients omit zones; preserve them, but honor explicit [] from current clients.
create function adventure_private.keep_board_zones() returns trigger language plpgsql set search_path='' as $$
begin
 if not (new.snapshot ? 'zones') and old.snapshot ? 'zones' then new.snapshot:=jsonb_set(new.snapshot,'{zones}',old.snapshot->'zones');end if;
 return new;
end $$;
revoke all on function adventure_private.keep_board_zones() from public,anon,authenticated;
create trigger board_keep_zones before update on public.board_saves for each row execute function adventure_private.keep_board_zones();
-- Initial framing follows the existing cast. No node, relationship, or other save part is replaced.
update public.board_saves set snapshot=jsonb_set(snapshot,'{zones}','[{"id":"ward-party","name":"冒险团","tone":"sage","x":40,"y":55,"width":910,"height":1045},{"id":"ward-village","name":"铁环村 · 矿坑线索","tone":"amber","x":1040,"y":85,"width":700,"height":1035}]'::jsonb)
where board_id='black-tower' and not (snapshot ? 'zones');
