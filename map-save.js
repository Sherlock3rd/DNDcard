(function(){
  'use strict';
  const table=window.MapTable,core=window.MAP_SAVE_CORE,button=document.querySelector('#mapSaveStatus');
  if(!button||!table)return;
  const demo=new URLSearchParams(location.search).has('showcase');
  if(demo){button.textContent='演示模式 · 不写入正式存档';button.disabled=true;return}
  let store,client,timer,authSequence=0,ready=false;
  const dialog=document.createElement('dialog');dialog.className='map-save-dialog';
  dialog.innerHTML=`<h2>地图存档</h2><p data-status role="status" aria-live="polite"></p><p data-error role="alert"></p>
    ...
    <section data-conflict hidden><p data-summary></p><p>两份地图会先保留备份，选择后作为后续同步版本。</p><button data-remote>使用云端版本</button><button data-local>保留本机版本</button></section>
    <div class="map-save-actions"><button data-retry>立即同步 / 重试</button><button data-download>下载当前地图</button><button data-backups>下载保留的副本</button><button data-close>关闭</button></div>`;
  document.body.append(dialog);const q=s=>dialog.querySelector(s);
  const text={local:'本机已保存 · 未登录云端',connecting:'本机已保存 · 正在连接云端',pending:'本机已保存 · 等待上传',syncing:'本机已保存 · 正在同步',synced:'云端已同步',empty:'账号暂无地图 · 编辑后自动上传',conflict:'本机已保存 · 存在版本冲突',editing:'本机已保存 · 完成编辑后读取云端',error:'本机已保存 · 云端未同步', 'storage-error':'本机保存失败 · 请勿关闭页面'};
  function update(s){
    ...
    q('[data-account]').textContent=s.user?'使用当前书房账号同步 · 私有地图':'尚未登录；本机编辑会保留';q('[data-login]').hidden=!!s.user;
    q('[data-conflict]').hidden=!s.entry.conflict;
    if(s.entry.conflict){const row=s.entry.conflict.row;q('[data-summary]').textContent=`本机 ${s.entry.snapshot.pieces.length} 枚棋子；云端 ${row?.snapshot.pieces.length||0} 枚棋子（版本 ${row?.revision||0}）。`;q('[data-remote]').disabled=!row}
  ...
  async function connect(){
    try{
      ...
      ready=true;await store.setUser(data.session?.user.id||null);
    }catch(e){ready=false;store?.report('error','账号连接失败：'+e.message)}
  }
  function start(){
    try{
      store=new core.Store({storage:localStorage,apply:table.apply,notify:update,canApply:table.canApply,remote:{async load(user){return (await request(user,'map_saves?select=*&user_id=eq.'+encodeURIComponent(user)+'&map_id=eq.faerun-3.5'))[0]||null},save(user,revision,snapshot){return request(user,'rpc/save_map',{p_map_id:'faerun-3.5',p_expected_revision:revision,p_snapshot:snapshot})}}});
      store.persist();document.querySelector('#viewport').inert=false;
      document.addEventListener('map:save',e=>{store.save(e.detail);if(ready)schedule()});
      ...
      setInterval(()=>{if(ready&&!document.hidden&&navigator.onLine)store.sync()},15000);
      connect();
    }catch(e){button.textContent='地图存档读取失败 · 请勿编辑';q('[data-error]').textContent=e.message;document.querySelector('#viewport').inert=true}
  }
  if(navigator.locks){navigator.locks.request('dndcard-map-editor-v1',{ifAvailable:true},lock=>{if(!lock){button.textContent='地图已在另一标签页打开 · 此页只读';q('[data-error]').textContent='请在原地图页继续编辑，关闭原页后刷新此页。';document.querySelector('#viewport').inert=true;return}start();return new Promise(resolve=>window.addEventListener('pagehide',resolve,{once:true}))});window.addEventListener('pageshow',e=>{if(e.persisted)location.reload()})}
  else start();
})();
