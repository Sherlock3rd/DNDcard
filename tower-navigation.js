(()=>{
 ...
 chrome.innerHTML='<a class="tower-wordmark" href="./index.html#portal">THE BLACK TOWER</a><nav class="tower-nav" aria-label="主要导航">'+[['room','书房',isIndex?'#portal':'./index.html#portal'],['board','案件板','./caseboard.html'],['map','地图桌','./map.html'],['character','人物',isIndex?'#overview':'./index.html#overview']].map(([id,label,href])=>`<a href="${href}" data-tower-page="${id}" title="${label}">${icon(id)}<span>${label}</span></a>`).join('')+'</nav>';
 ...
 panel.addEventListener('click',e=>{if(e.target.closest('#boardNew,#boardNewZone,#boardConnect,#toggle,#journalPlacesToggle,#mapSaveStatus,[data-open-archive],[data-open-bookshelf],[data-open-journal]'))setOpen(false)});
 if(kind==='board'){
  ...
  pane('room').innerHTML='<button type="button" data-open-bookshelf>书架与设置</button><button type="button" data-open-journal>冒险日记</button>';
