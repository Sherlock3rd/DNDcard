(()=>{
 ...
 chrome.innerHTML='<a class="tower-wordmark" href="./index.html#portal">THE BLACK TOWER</a><nav class="tower-nav" aria-label="主要导航">'+[['room','书房',isIndex?'#portal':'./index.html#portal'],['board','案件板','./caseboard.html'],['map','地图桌','./map.html'],['character','人物',isIndex?'#overview':'./index.html#overview'],['journal','冒险日记','./journal.html']].map(([id,label,href])=>`<a href="${href}" data-tower-page="${id}" title="${label}">${icon(id)}<span>${label}</span></a>`).join('')+'</nav>';
