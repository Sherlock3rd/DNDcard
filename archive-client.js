(()=>{
 ...
 copyLink.onclick=async()=>{const key=owner?.getKey();if(!key)return;const url=new URL('index.html',config.redirectUrl);url.hash='owner='+key;try{await navigator.clipboard.writeText(url.href);copyLink.textContent='已复制 · 在其他设备打开即可'}catch{linkField.value=url.href;linkField.hidden=false;linkField.select()}};
 ...
 function update(){
  ...
  const text=bad?label[bad.status]:loading?'读取最新冒险档案…':ownerError?'专属入口待恢复 · 本机修改保留':pending?(canWrite()?'本机已保存 · 正在同步':'本机已保存 · 打开专属入口后同步'):ownerReady?'自动同步已开启':user?'冒险档案已同步':'最新冒险档案 · 公开查看';
  ...
  q('[data-summary]').textContent=text+(latest?' · 档案版本 '+latest.revision:'');q('[data-error]').textContent=bad?.error||ownerError||'';
  ...
  q('[data-auth]').hidden=canWrite()||!!owner?.getKey();q('[data-identity]').textContent=ownerReady?'专属入口已启用 · 无需邮箱密码':user?'已登录：'+user.email:'通过专属链接进入，可免登录自动同步。';q('[data-logout]').hidden=!user&&!owner?.getKey();q('[data-logout]').textContent=owner?.getKey()?'移除此设备的免登录入口':'退出本机登录';copyLink.hidden=!ownerReady;
 ...
 async function json(url,options={}){const r=await fetch(url,{cache:'no-store',...options,signal:AbortSignal.timeout(12000)});if(!r.ok){const error=Error('请求暂不可用（'+r.status+'）');error.status=r.status;throw error}return r.json()}
 async function ownerRequest(body){return json(config.url+'/rest/v1/rpc/access_adventure',{method:'POST',headers:{apikey:config.publishableKey,'Content-Type':'application/json'},body:JSON.stringify({p_key:owner.getKey(),...body})})}
 async function checkOwner(){
  if(!owner?.getKey())return;if(ownerChecking)return ownerChecking;if(ownerReady&&Date.now()-ownerChecked<60000)return;
  ownerChecking=(async()=>{try{const r=await ownerRequest({});if(r.status!=='ready')throw Error('专属入口暂不可用');ownerReady=true;ownerChecked=Date.now();ownerError=owner.error||''}catch(e){ownerReady=false;ownerError=[401,403].includes(e.status)?'专属链接已失效，请重新打开有效的专属入口。':'暂时无法验证专属入口，联网后会自动重试。'}for(const s of stores.values())s.authorized=canWrite();update()})().finally(()=>{ownerChecking=null});return ownerChecking;
 }
 ...
 async function savePart(name,revision,snapshot){
  if(owner?.getKey()){
   try{const result=await ownerRequest({p_part:name,p_expected_revision:revision,p_snapshot:snapshot});if(result.status==='saved')latest=null;return result}
 ...
 async function syncAll(){await checkOwner();await Promise.all([...stores.values()].map(s=>s.sync()));try{await getLatest()}catch{}checkGit()}
 ...
 q('[data-logout]').onclick=async()=>{try{owner?.forget();ownerReady=false;ownerError='';linkField.value='';linkField.hidden=true;await client?.auth.signOut({scope:'local'});await auth(null)}catch{q('[data-error]').textContent='未能移除此设备的入口，请重试。'}};dialog.addEventListener('close',()=>{q('[name=password]').value='';linkField.value='';linkField.hidden=true });
 ...
 (async()=>{try{client=window.DND_SHARED_CLOUD_CLIENT||(window.DND_SHARED_CLOUD_CLIENT=window.DND_SUPABASE.createClient(config.url,config.publishableKey,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}}));client.auth.onAuthStateChange((_event,session)=>{setTimeout(()=>auth(session?.user),0)});if(!owner?.getKey()){const {data,error}=await client.auth.getSession();if(error)throw error;await auth(data.session?.user)}}catch(e){q('[data-error]').textContent='账号连接暂不可用，仍可读取公开存档'}started=true;await syncAll()})();
})();
